...
public class EmployeeController {
    ...
    public ResponseEntity<Page<EmployeeResponse>> getEmployees(
            @Parameter(description = "Từ khóa tìm kiếm (tên, email, SĐT)", example = "Nguyễn")
            @RequestParam(required = false) String keyword,

            @Parameter(description = "Lọc theo ID chi nhánh", example = "1")
            @RequestParam(required = false) Long branchId,

            @Parameter(description = "Lọc theo ID vai trò", example = "2")
            @RequestParam(required = false) Long roleId,

            @Parameter(description = "Lọc theo trạng thái (ACTIVE, INACTIVE, BANNED)", example = "ACTIVE")
    ...
    @Operation(
            ...
            description = "Tạo tài khoản nhân viên mới với thông tin cá nhân, chi nhánh, và vai trò. " +
                    "Mật khẩu mặc định là 'Agri@2024' nếu không được cung cấp.",
            responses = {
                    @ApiResponse(
                            responseCode = "201",
